@props(['job', 'endDate' => false, 'showCompany' => false, 'showEducation' => false])
...
<article class="bg-white rounded-lg shadow-sm p-6 border-l-4 border-blue-500">
  <div class="flex items-start justify-between mb-3">
    <div>
      <h3 class="text-lg font-bold text-gray-700">
        @if ($showCompany)
          {{ $job->company->nombre_empresa ?? '' }} - {{ $job->titulo }}
        @else
          {{ $job->titulo }}
        @endif
      </h3>
  ...
  <div class="flex items-center justify-between ">
    <div class="flex items-center gap-10">
      <p class="text-lg font-bold text-gray-700"> ${{ number_format($job->salario, 0, ',', '.') }}</p>
