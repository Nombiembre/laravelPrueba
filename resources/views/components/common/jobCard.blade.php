@props(['job', 'endDate' => false])

<article class="bg-white rounded-lg shadow-sm p-6 border-l-4 border-blue-500">
  <div class="flex items-start justify-between mb-3">
    <div>
      <h3 class="text-lg font-bold text-gray-900">{{ $job->company->nombre_empresa ?? '' }} - {{ $job->titulo }}</h3>
  ...
  @if ($endDate)
    <div class="flex items-center justify-between mt-4">
      <div class="flex gap-1 text-gray-500 text-sm">
        ...
        <span>Cierra el {{ \Carbon\Carbon::parse($job->created_at)
       ->locale('es') 
       ->isoFormat('D [de] MMMM [de] YYYY') }}</span>
      </div>
    </div>
  @else
  @endif
